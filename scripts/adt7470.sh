...
do_instantiate adt7470 ${i2c_addr} 2>/dev/null
getbasedir ${i2c_addr}
...
for f in $(seq 1 4)
do
	check_range -b ${basedir} -l 1 -u 50000 -d 400 -q -n fan${f}_min
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 1 -u 50000 -d 400 -q -n fan${f}_max
	rv=$(($? + ${rv}))
done

for p in $(seq 1 4)
do
	check_range -b ${basedir} -l 0 -u 255 -r -d 0 -q -n pwm${p}
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 255 -r -d 0 -q -n pwm${p}_auto_point1_pwm
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 255 -r -d 0 -q -n pwm${p}_auto_point2_pwm
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -q pwm${p}_auto_point1_temp
	rv=$(($? + ${rv}))
done

check_range -b ${basedir} -d 0 -r -q num_temp_sensors
rv=$(($? + ${rv}))
check_range -b ${basedir} -d 0 -r -q auto_update_interval
rv=$(($? + ${rv}))
check_range -b ${basedir} -l 0 -u 1 -d 0 -r -q -n force_pwm_max
rv=$(($? + ${rv}))
