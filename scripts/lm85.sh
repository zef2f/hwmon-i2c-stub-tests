...
do_tests()
{
    ...
    for t in $(seq 1 3)
    do
	auto_temp_min=$(cat temp${t}_auto_temp_min)
	auto_temp_max_min=$((auto_temp_min + 2000))
	auto_temp_max_max=$((auto_temp_min + 80000))
	auto_temp_off_min=$((auto_temp_min - 15000))
	auto_temp_off_max=${auto_temp_min}

	check_range -b ${basedir} -d 500 -r -q temp${t}_min
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -r -q temp${t}_max
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -r -q temp${t}_auto_temp_crit
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l ${auto_temp_max_min} -u ${auto_temp_max_max} -n -d 14000 -r -q temp${t}_auto_temp_max
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -r -q temp${t}_auto_temp_min
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l ${auto_temp_off_min} -u ${auto_temp_off_max} -n -d 500 -r -q temp${t}_auto_temp_off
	rv=$(($? + ${rv}))
    done

    for f in $(seq 1 4)
    do
	check_range -b ${basedir} -l 0 -u 20000 -n -d 52 -q fan${f}_min
	rv=$(($? + ${rv}))
    done

    for i in $(seq 0 4)
    do
	check_range -b ${basedir} -r -q -d 32 in${i}_min
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -r -q -d 32 in${i}_max
	rv=$(($? + ${rv}))
    done

    for p in $(seq 1 3)
    do
	check_range -b ${basedir} -l 0 -u 255 -n -r -q -d 0 pwm${p}
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 255 -n -r -q -d 0 pwm${p}_auto_pwm_min
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 255 -n -r -q -d 0 pwm${p}_auto_pwm_minctl
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 2 -r -q -d 0 pwm${p}_enable
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -r -q -d 20 pwm${p}_freq
	rv=$(($? + ${rv}))
    done

    check_range -b ${basedir} -l 0 -u 255 -r -q -d 0 vrm
    rv=$(($? + ${rv}))
