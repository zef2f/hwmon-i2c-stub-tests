...
overflow_check_val()
{
    ...
    omax=$(cat ${attr})
    # The returned value may be lower than the written value.
    # That may mean it was truncated. That does not automatically
    # mean that there was an overflow. Assume overflow if the
    # returned value is less than half than the written value.
    if [ ${omax} -ne ${max} -a "${omax}" -lt "$((overflow / 2))" ]
    then
...
load_i2c_stub()
{
    ...
    i2c_adapter="$(find_stub_i2c_adapter)"
...
find_stub_i2c_adapter()
{
    local root
    local match
    local path

    for root in /sys/class/i2c-dev /sys/class/i2c-adapter
    do
	[ -d "${root}" ] || continue
	match="$(grep -H "SMBus stub driver" "${root}"/*/name 2>/dev/null | head -n 1)"
	if [ -n "${match}" ]; then
	    path="${match%%:*}"
	    basename "$(dirname "${path}")" | cut -f2 -d-
	    return 0
	fi
    done
