...
do_instantiate adt7462 ${i2c_addr} 2>/dev/null
getbasedir ${i2c_addr}
...
# Other voltages are disabled in this register image.
for i in $(seq 1 2)
do
	check_range -b ${basedir} -d 32 -q "in${i}_min"
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 32 -q "in${i}_max"
	rv=$(($? + ${rv}))
done

# temp4 is disabled in this register image.
for t in $(seq 1 3)
do
	check_range -b ${basedir} -d 500 -q temp${t}_min
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -q temp${t}_max
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -q temp${t}_auto_point1_hyst
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -q temp${t}_auto_point1_temp
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -q temp${t}_auto_point2_hyst
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -q -S temp${t}_auto_point2_temp
	rv=$(($? + ${rv}))
done

# Only fans 7 and 8 are writable in this image.
for f in $(seq 7 8)
do
	check_range -b ${basedir} -l 1000 -u 10000 -d 4000 -n -q fan${f}_min
	rv=$(($? + ${rv}))
done

for p in $(seq 1 4)
do
	check_range -b ${basedir} -l 0 -u 255 -r -d 0 -n -q pwm${p}
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 8 -u 9 -d 0 -q pwm${p}_auto_channels_temp
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 255 -r -d 0 -n -q pwm${p}_auto_point1_pwm
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 255 -r -d 0 -n -q pwm${p}_auto_point2_pwm
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 1 -d 0 -q pwm${p}_enable
	rv=$(($? + ${rv}))
done

check_range -b ${basedir} -l 0 -u 1 -d 0 -r -n -q force_pwm_max
rv=$(($? + ${rv}))
