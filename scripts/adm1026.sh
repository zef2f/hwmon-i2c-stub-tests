...
do_instantiate adm1026 ${i2c_addr}
getbasedir ${i2c_addr}
...
for i in $(seq 0 16)
do
	# in8 and in9 are not available in this register image
	if [ $i -eq 8 -o $i -eq 9 ]
	then
		continue
	fi
	check_range -b ${basedir} -d 36 -r -q "in${i}_min"
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 36 -r -q "in${i}_max"
	rv=$(($? + ${rv}))
done
...
do
	...
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 1 -r -q temp${i}_crit_enable
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -r -q temp${i}_crit
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -d 500 -r -q temp${i}_offset
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -d 0 -r -q temp${i}_auto_point1_pwm
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -r -q temp${i}_auto_point1_temp
	rv=$(($? + ${rv}))
done
...
do
	...
	check_range -b ${basedir} -l 0 -u 255 -d 0 -n -r -q pwm${i}
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 2 -r -q pwm${i}_enable
	rv=$(($? + ${rv}))
done

for i in $(seq 1 8)
do
	check_range -b ${basedir} -l 1 -u 2 -d 0 -n -r -q fan${i}_div
	rv=$(($? + ${rv}))
	check_range -b ${basedir} -l 0 -u 20000 -d 2500 -n -r -q fan${i}_min
	rv=$(($? + ${rv}))
done

check_range -b ${basedir} -d 8 -r -q analog_out
rv=$(($? + ${rv}))
check_range -b ${basedir} -d 0 -l 0 -u 131071 -n -r -q gpio
rv=$(($? + ${rv}))
check_range -b ${basedir} -d 0 -l 0 -u 131071 -n -r -q gpio_mask
rv=$(($? + ${rv}))
check_range -b ${basedir} -d 0 -l 0 -u 65535 -n -r -q alarm_mask
rv=$(($? + ${rv}))
